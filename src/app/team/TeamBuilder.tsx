...
function PitchPreview({
  ...
  return (
    <div className="pitch-lines flex flex-col justify-between gap-2 rounded-xl bg-pitch p-3 sm:p-4">
      {rows.map(({ pos }) => (
        <div key={pos} className="flex flex-wrap justify-center gap-2">
          {selectedPlayers
            ...
            .map((p) => (
              <div
                key={p.id}
                className="flex min-w-[64px] flex-col items-center rounded-lg bg-white/95 px-2 py-1.5 text-center shadow"
              >
                <span className="text-xs font-semibold text-pitch-dark">
                  {p.name.split(" ").slice(-1)[0]}
                  {captainId === p.id && <span className="ml-1 text-gold">C</span>}
                </span>
              </div>
            ))}
